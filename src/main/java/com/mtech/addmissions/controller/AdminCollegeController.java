...
public class AdminCollegeController {
    ...
    @GetMapping()
    public ResponseEntity<List<CollegeListDTO>> getAll() {
        List<CollegeListDTO> response = collegeServiceImp.getAllColleges();
    ...
    @PutMapping()
    public ResponseEntity<CollegeListDTO> update(@RequestBody CollegeDTO college) throws ResourseNotExist {
        return new ResponseEntity<>(collegeServiceImp.update(college), HttpStatus.OK);
    }

    @DeleteMapping("/{collegeCode}")
    public ResponseEntity<?> delete(@PathVariable String collegeCode) throws ResourseNotExist {
